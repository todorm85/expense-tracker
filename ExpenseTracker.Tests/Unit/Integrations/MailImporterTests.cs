...
namespace ExpenseTracker.Integrations.Tests
{
    ...
    public class MailImporterTests
    {
        ...
        public void ImportsTransactionsCorrectly()
        {
            ...
            importer.ImportTransactions(out IEnumerable<Transaction> ts, out IEnumerable<CreateTransactionResult> skipped, out IEnumerable<ImportError> errors);
            ...
            importer.ImportTransactions(out IEnumerable<Transaction> ts2, out IEnumerable<CreateTransactionResult> skipped2, out IEnumerable<ImportError> errors2);
            Assert.AreEqual(2, transactionsList.Count);
            Assert.AreEqual("dummy1", transactionsList[0].Details);
            Assert.AreEqual("dummy2", transactionsList[1].Details);
            Assert.AreEqual(2, msgList.Count);
